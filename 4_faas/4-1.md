...
def handler(ctx, data: io.BytesIO = None):
    try:
        ...
        logging.getLogger().info('error parsing json payload: ' + str(ex))
        message = "こんにちは"  # JSON解析エラーがあった場合のデフォルトメッセージ
...
Using Container engine docker
Building image nrt.ocir.io/nrruovdjeqty/myapp-repo/func-p:0.0.7 TargetedPlatform:  amd64HostPlatform:  amd64
..
Using Container engine  docker  to push
Pushing nrt.ocir.io/nrruovdjeqty/myapp-repo/func-p:0.0.7 to docker registry...The push refers to repository [nrt.ocir.io/nrruovdjeqty/myapp-repo/func-p]
392431edb7ff: Pushed 
b6f38abf4314: Pushed 
930e1739e6b9: Layer already exists 
2bac13f3b124: Layer already exists 
...
8f580ded3ec7: Layer already exists 
db3dbcc68409: Layer already exists 
0.0.7: digest: sha256:22b9f02932e15932c677d2d52ff9c3d9a5ddbcc9c90df7b4d0e7947ecf921001 size: 1780
Updating function func-p using image nrt.ocir.io/nrruovdjeqty/myapp-repo/func-p:0.0.7...
[opc@admin2-vm1 func-p]$ fn invoke myapp func-p
{"message": "\u3053\u3093\u306b\u3061\u306f"}
[opc@admin2-vm1 func-p]$ fn invoke myapp func-p | jq
{
  "message": "こんにちは"
}
[opc@admin2-vm1 func-p]$ 
